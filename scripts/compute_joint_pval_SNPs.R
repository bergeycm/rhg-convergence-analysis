...
# ----------------------------------------------------------------------------------------
# --- Find convergent SNPs (e.g. high PBS in both Batwa and Andamanese)
# ----------------------------------------------------------------------------------------

library(xtable)
library(metap)

# --- Read in SNP data with (empirical) p-values

stats = list()

stats[['pbs.batwa']] =
                        read.table("reports/pbs.Batwa.GBR.anno.extreme.vals.all.txt",
                             header=TRUE, fill=TRUE)
stats[['pbs.bakiga']] =
                        read.table("reports/pbs.Bakiga.GBR.anno.extreme.vals.all.txt",
                             header=TRUE, fill=TRUE)
stats[['pbs.anda']] =
                        read.table("reports/pbs.Anda.LWK.anno.extreme.vals.all.txt",
                             header=TRUE, fill=TRUE)
stats[['pbs.br1']] =
                        read.table("reports/pbs.BR1.LWK.anno.extreme.vals.all.txt",
                             header=TRUE, fill=TRUE)
...
compute.joint.pval = function (stat1, stat2) {
    ...
    stats.both$del.any = FALSE
    stats.both[grepl("DEL", stats.both$anno.x) |
               grepl("DEL", stats.both$anno.y),]$del.any = TRUE
...
cols.to.keep = c(5,15,6,8,16,12,14,17:18)

combine.joint.info = function (joint.stats) {

    snp.info = paste0(joint[[joint.stats]][,1], ":", joint[[joint.stats]][,2])

    full = cbind(snp.info, joint[[joint.stats]][,cols.to.keep])
...
write.table(full, file="results/joint_SNP_pvalues.txt")

del.any = grepl("DEL", full$anno.x) | grepl("DEL", full$anno.y)

cleaned     = full[which(          full$joint.p.edington < 0.0001),]
cleaned.del = full[which(del.any & full$joint.p.edington < 0.001),]
...
write.to.latex = function (df, out.file) {
    ...
    names(df) = c("SNP", "Genes",
        "Stat 1", "Stat 1 $p$", " ",
        "Stat 2", "Stat 2 $p$", " ",
        "$p$ Edgington", "$p$ Fisher")

    xt = xtable(df, digits=c(0,0,0,0,-3,0,0,-3,0,-3,-3), align='lll|lrr|lrr|rr')
...
write.to.latex(df = cleaned,     out.file = "reports/joint_SNP_pvalues.tex")
write.to.latex(df = cleaned.del, out.file = "reports/joint_SNP_pvalues.deleterious.tex")
